...
static void set_covered_main_tree(const MatOptimize::MAT::Node *start,
                                  std::vector<char> &checked, int distance_left,
                                  const MatOptimize::MAT::Node *exclude_node) {
...
check_mutation_helper(Mutation_Set &ref,
                      const MatOptimize::MAT::Mutations_Collection &follows,
    ...
    for (const auto &mut : follows) {
        ...
        if (mut.mut_nuc==0xf) {
            for(int pos=mut.position; pos<=mut.get_end_range(); pos++) {
                auto ref_iter = ref.find(MatOptimize::MAT::Mutation(mut.chrom_idx,pos,0,0xf));
                if (ref_iter != ref.end()) {
        ...
        auto ref_iter = ref.find(MatOptimize::MAT::Mutation(mut.chrom_idx,mut.position,mut.par_nuc,mut.mut_nuc));
        if (ref_iter != ref.end()) {
...
void check_ancestor(const MatOptimize::MAT::Node* parent_node,Mutation_Set &ref,std::unordered_map<int, uint8_t>& parent_allele_check) {
    ...
    /*for (const auto &temp : parent_allele_check) {
        if (MatOptimize::MAT::Mutation::refs[temp.first] != temp.second) {
            raise(SIGTRAP);
        }
    }*/
}
void check_mutations(Mutation_Set ref,
                     const MatOptimize::MAT::Mutations_Collection &sample_mutations,
                     const MatOptimize::MAT::Mutations_Collection &shared_mutations,
                     const MatOptimize::MAT::Node *parent_node) {
    std::unordered_map<int, uint8_t> parent_allele_check;
    parent_allele_check.reserve(MatOptimize::MAT::Mutation::refs.size());
...
void check_continuation(const MatOptimize::MAT::Node* parent_node,Mutation_Set ref,const std::vector<To_Place_Sample_Mutation> &decendent_mutations) {
