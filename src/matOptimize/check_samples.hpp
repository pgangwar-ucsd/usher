...
struct Mutation_Pos_Only_Comparator {
    bool operator()(const MatOptimize::Mutation_Annotated_Tree::Mutation &first,
                    const MatOptimize::Mutation_Annotated_Tree::Mutation &second) const {
        return (first.get_position() == second.get_position());
    }
};
struct Mutation_Pos_Only_Hash {
    size_t operator()(const MatOptimize::Mutation_Annotated_Tree::Mutation &in) const {
        return in.get_position();
    }
};
struct Node_Idx_Hash {
    size_t operator()(const MatOptimize::Mutation_Annotated_Tree::Node* in) const {
        return in->bfs_index;
    }
};

struct Node_Idx_Eq {
    bool operator()(const MatOptimize::Mutation_Annotated_Tree::Node* first,const MatOptimize::Mutation_Annotated_Tree::Node* second)const {
        return first->bfs_index==second->bfs_index;
    }
};

typedef std::unordered_set<MatOptimize::Mutation_Annotated_Tree::Mutation, Mutation_Pos_Only_Hash, Mutation_Pos_Only_Comparator> Mutation_Set;
...
void check_samples(
    const MatOptimize::Mutation_Annotated_Tree::Node *root,
    Original_State_t &samples,const MatOptimize::Mutation_Annotated_Tree::Tree* tree,bool ignore_missed_samples=false);
