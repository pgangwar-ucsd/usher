...
struct Mapper_Cont {
    std::vector<ripples::Ripples_Mapper_Mut> muts;
};
void prep_output(const ripples::Pruned_Sample &sample,std::vector<ripples::Ripples_Mapper_Mut> &init,
                 ripples::Ripples_Mapper_Output_Interface &out, size_t node_size) {
...
struct Mapper_Op_Common {
		ripples::Ripples_Mapper_Output_Interface &out;
    const size_t stride;
    const size_t mut_count;
    const std::vector<int>& idx_map;
    const std::vector<bool>& do_parallel;
    const std::vector<ripples::Mapper_Info> &traversal_track;
    const int skip_idx;
    const unsigned short tree_height;
		ripples::Mut_Count_t& get_mut_count_ref(size_t node_idx,unsigned short mut_idx) {
        return out.mut_count_out[mut_idx*stride+node_idx];
    }
		ripples::Mut_Count_t& get_final_mut_count_ref(size_t node_idx) {
...
std::pair<const ripples::MAT::Node*,ripples::MAT::Mutation> get_parent_mut(const ripples::MAT::Node* node,int pos) {
    node=node->parent;
    while (node!=nullptr) {
    ...
    return std::make_pair(nullptr,ripples::MAT::Mutation());
}

static void only_from_parent(const ripples::Ripples_Mapper_Mut& mut,size_t this_idx,unsigned short& mut_accumulated,std::vector<ripples::Ripples_Mapper_Mut>& this_mut_out, Mapper_Op_Common& cfg) {
...
void set_mut_count(int this_idx,
                   std::vector<ripples::Ripples_Mapper_Mut> &this_mut_out,
                   const ripples::MAT::Mutation *this_mut_start,
                   const ripples::MAT::Mutation *this_mut_end, bool is_leaf,
                   const std::vector<ripples::Ripples_Mapper_Mut> &parent_muts,
...
void merge_mutation_only(std::vector<ripples::Ripples_Mapper_Mut>& this_mut_out,const ripples::MAT::Node* node,const std::vector<ripples::Ripples_Mapper_Mut>& parent_muts) {
    auto iter = parent_muts.begin();
    auto end = parent_muts.end();
...
static void
serial_mapper(const std::vector<ripples::Ripples_Mapper_Mut> &parent_muts,
              Mapper_Op_Common &cfg,
              int start_idx) {
    const std::vector<ripples::Mapper_Info> &traversal_track=cfg.traversal_track;
    std::vector<std::vector<ripples::Ripples_Mapper_Mut>> stack;
...
struct Mapper_Op {
    const std::vector<ripples::Ripples_Mapper_Mut> &parent_muts;
    Mapper_Op_Common& cfg;
    const ripples::MAT::Node *node;
    Mapper_Op(const std::vector<ripples::Ripples_Mapper_Mut> &parent_muts,
              Mapper_Op_Common& cfg,
              const ripples::MAT::Node *node)
...
namespace ripples {
void ripples_mapper(const ripples::Pruned_Sample &sample,
                    ripples::Ripples_Mapper_Output_Interface &out,
                    size_t node_size,
                    const std::vector<int>& idx_map,
                    const std::vector<bool>& do_parallel,
                    const std::vector<ripples::Mapper_Info> &traversal_track,
                    const unsigned short tree_height,
                    const ripples::MAT::Node *root,
                    const ripples::MAT::Node *skip_node) {
    auto temp=get_parent_mut(root,0);
    auto mut_size = sample.sample_mutations.size();
    std::vector<ripples::Ripples_Mapper_Mut> root_muts;
